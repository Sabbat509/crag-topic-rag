#!/usr/bin/env python3
"""Evaluate CRAG with Kiraffe-style BERTopic topic partitioning on HotpotQA."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CRAG + Kiraffe-style topic RAG on HotpotQA.")
    parser.add_argument("--index-dir", type=Path, default=Path("result/crag_topic_rag/topic_index"))
    parser.add_argument("--data-file", type=Path, default=Path("data/hotpot_dev_fullwiki_v1.json"))
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("result/crag_topic_rag/hotpotqa_eval/reference_crag_topic_fullwiki"))
...
def build_prompt(question: str, retrieved_chunks: list[dict[str, Any]]) -> str:
    ...
    return f"""Use only the context below to answer the HotpotQA question.
Return the shortest possible answer only.
Return only the entity, name, date, number, yes, or no.
Do not write a sentence.
Do not explain.
Do not include citations.
If the answer is not in the context, answer exactly: no
...
def append_csv_row(path: Path, row: dict[str, Any]) -> None:
    fieldnames = [
        "id", "question", "gold_answer", "prediction", "em", "acc", "f1", "g_sem", "token",
...
def write_summary(path: Path, rows: list[dict[str, Any]], config: dict[str, Any]) -> None:
    ...
    summary = {
        ...
        "metrics": {
            ...
            "G-Sem": sum(row["g_sem"] for row in rows) / count if count else 0.0,
            "Token": sum(row["token"] for row in rows) / count if count else 0.0,
        },
        "crag": {
    ...
    with records_path.open("a", encoding="utf-8") as records_file:
        for ordinal, index in enumerate(selected_indices, 1):
            ...
            row = {
                ...
                "acc": relaxed_accuracy(prediction, gold_answer),
                "f1": token_f1(prediction, gold_answer),
                "g_sem": semantic_similarity(embedding_model, prediction, gold_answer),
                "token": len(answer_tokens(prediction)),
